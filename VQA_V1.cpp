...
int main(int argc, const char* argv[])
{
    if (argc > 1) {
        ...
        std::vector<std::string> compsName;
        for (int i = 1; i < argc - 1; i++) {
            std::string path = argv[i+1];
            std::string base_filename = path.substr(path.find_last_of("/\\") + 1);
            compsName.push_back(base_filename);
        }
        ...
        std::string folderName = str.substr(0, found);
        folderName = folderName.substr(folderName.find_last_of("/\\") + 1);
        myfile.open(folderName + "_output.csv");
        std::string firstline;
        for (int i = 0; i < argc - 2;i++) {
            firstline = firstline + "Frame,PSNR " + compsName[i] + ",";
        }
        firstline = firstline + "\n";
        myfile << firstline;
        ...
        for (int i = 0; i < reslength-1;i++) {
            ...
            for (int j = 0; j < argc - 2;j++) {
                int frame = delay[j+1] + i;
                line = line + std::to_string(frame) +","+ std::to_string(res[i][j]) + ",";
            }
            line = line + "\n";
            myfile << line;
        }
